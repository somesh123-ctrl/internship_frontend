...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!email) return toast.warning("All fields are required");

    const res = await axios.post(
      "https://internship-backend-gjjh.onrender.com/api/v1/sendotp",
      {
        email: email,
      }
    );
  ...
  const handleOtpSubmit = async () => {
    ...
    const res = await axios.post(
      "https://internship-backend-gjjh.onrender.com/api/v1/signup",
      {
        email: email,
        otp: userOtp,
      }
        ...
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative w-64 p-6 bg-white rounded">
              <h2 className="mb-4 text-xl font-bold">Enter OTP</h2>
              <input
                type="text"
                className="w-full px-3 py-2 rounded"
                placeholder="OTP"
              ...
              <button
                type="submit"
                className="px-4 py-2 mt-4 mr-2 font-bold text-black transition-all duration-300 bg-red-500 rounded hover:bg-red-700"
                onClick={() => {
                  setShowModal(false);
                }}
              >
              ...
              <button
                type="submit"
                className="px-4 py-2 mt-4 font-bold text-black transition-all duration-300 bg-blue-500 rounded hover:bg-blue-700"
